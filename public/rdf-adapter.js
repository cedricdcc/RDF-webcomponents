...
function getIndex({ subject, predicate, object }, set) {
  ...
  const o6 = object.value || set.has(object) || (set.add(object), false);
  return !s4 && p3 ? { content: "predicates", value: [predicate, object, subject] } : o6 ? { content: "objects", value: [object, subject, predicate] } : { content: "subjects", value: [subject, predicate, object] };
}
    ...
    N3Reasoner = class {
      ...
      _reasonGraphNaive(rules, content) {
        ...
        while ((r5 = newRules.pop()) !== void 0) {
          const [subject, predicate, object, rule] = r5;
...
// src/rdf-webcomponents/components/rdf-adapter.ts
var RdfAdapter = class extends i4 {
  // ===========================================================================
  // Lifecycle Methods
  // ===========================================================================
  constructor() {
  ...
  async refresh() {
    if (this._worker) {
      await this._worker.send("CACHE_CLEAR_REQUEST" /* CACHE_CLEAR_REQUEST */, {});
    }
    await this.reload();
  }
  disconnectedCallback() {
  ...
  updated(changedProperties) {
    ...
    if (changedProperties.has("url") || changedProperties.has("strategy") || changedProperties.has("subject") || changedProperties.has("subjectQuery") || changedProperties.has("subjectClass")) {
      if (this.url) {
        this._fetchData();
      }
    }
  ...
  _initWorker() {
    ...
    try {
      ...
      this._worker = new WorkerMessenger(worker, 6e4);
  ...
  async _fetchAndParseRdf(options) {
  ...
  async _parseRdf(content, format, baseUrl) {
    const { Parser } = await Promise.resolve().then(() => (init_src(), src_exports));
    const parser = new Parser({
      format: format === "turtle" ? void 0 : format,
      baseIRI: baseUrl
    });
    ...
    return new Promise((resolve, reject) => {
      ...
      parser.parse(content, (error, quad2) => {
        if (error) {
          ...
          quads.push({
            subject: this._serializeTerm(quad2.subject),
            predicate: this._serializeTerm(quad2.predicate),
            object: this._serializeTerm(quad2.object),
            graph: quad2.graph ? this._serializeTerm(quad2.graph) : void 0
          });
        } else {
...
], RdfAdapter.prototype, "url", 2);
__decorateClass([
  n4({ type: String, reflect: true })
], RdfAdapter.prototype, "format", 2);
__decorateClass([
  n4({ type: String, reflect: true })
], RdfAdapter.prototype, "strategy", 2);
__decorateClass([
  n4({ type: String, reflect: true })
], RdfAdapter.prototype, "subject", 2);
__decorateClass([
  n4({ type: String, attribute: "subject-query" })
], RdfAdapter.prototype, "subjectQuery", 2);
__decorateClass([
  n4({ type: String, attribute: "subject-class" })
], RdfAdapter.prototype, "subjectClass", 2);
__decorateClass([
  n4({ type: Number, reflect: true })
], RdfAdapter.prototype, "depth", 2);
__decorateClass([
  n4({ type: String, reflect: true })
], RdfAdapter.prototype, "graph", 2);
__decorateClass([
  n4({ type: String, reflect: true })
], RdfAdapter.prototype, "cache", 2);
__decorateClass([
  n4({ type: Number, attribute: "cache-ttl" })
], RdfAdapter.prototype, "cacheTtl", 2);
__decorateClass([
  n4({ type: Boolean, reflect: true })
], RdfAdapter.prototype, "shared", 2);
__decorateClass([
  n4({ type: String, reflect: true })
], RdfAdapter.prototype, "headers", 2);
RdfAdapter = __decorateClass([
  t3("rdf-adapter")
], RdfAdapter);
export {
  RdfAdapter
